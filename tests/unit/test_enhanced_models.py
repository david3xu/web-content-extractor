...
class TestExtractedLink:
    ...
    def test_domain_methods(self) -> None:
    ...
    def test_pdf_factory(self) -> None:
        """Test PDF link factory method."""
        link = ExtractedLink.create_pdf_link(
            "https://example.com/doc.pdf", "Test Document"
        )
    ...
    def test_pdf_factory_with_pdf_in_url(self) -> None:
        ...
        with pytest.raises(ValueError, match="URL does not appear to be a PDF"):
            ExtractedLink.create_pdf_link(
                "https://example.com/files/pdf/document", "Test Document"
            )

    def test_youtube_factory(self) -> None:
        """Test YouTube link factory method."""
        link = ExtractedLink.create_youtube_link(
            "https://youtube.com/watch?v=123", "Test Video"
        )
    ...
    def test_youtube_factory_short_url(self) -> None:
        """Test YouTube factory with short URL."""
        link = ExtractedLink.create_youtube_link(
            "https://youtu.be/xyz789", "Test Video"
        )

        assert link.link_type == LinkType.YOUTUBE

    def test_youtube_factory_embed_url(self) -> None:
        """Test YouTube factory with embed URL."""
        link = ExtractedLink.create_youtube_link(
            "https://youtube.com/embed/def456", "Test Video"
        )

        assert link.link_type == LinkType.YOUTUBE

    def test_other_factory(self) -> None:
        """Test generic link factory method."""
        link = ExtractedLink.create_other_link("https://example.com/page", "Test Page")

        assert link.link_type == LinkType.OTHER
        assert link.link_text == "Test Page"

    def test_other_factory_default_text(self) -> None:
        """Test generic link factory with default text."""
        link = ExtractedLink.create_other_link("https://example.com/page", "")

        assert link.link_type == LinkType.OTHER
        assert link.link_text == "https://example.com/page"

    def test_invalid_pdf_factory(self) -> None:
        """Test PDF factory with invalid URL."""
        with pytest.raises(ValueError, match="URL does not appear to be a PDF"):
            ExtractedLink.create_pdf_link(
                "https://example.com/notapdf.html", "Not a PDF"
            )

    def test_invalid_youtube_factory(self) -> None:
        """Test YouTube factory with invalid URL."""
        with pytest.raises(
            ValueError, match="URL does not appear to be a YouTube link"
        ):
    ...
    def test_url_depth_calculation(self) -> None:
...
class TestExtractionMetadata:
    ...
    def test_link_distribution(self) -> None:
    ...
    def test_link_distribution_zero_links(self) -> None:
    ...
    def test_content_rich_detection(self) -> None:
...
class TestExtractionResult:
    ...
    def test_domain_methods(self) -> None:
        ...
        pdf_link = ExtractedLink.create_pdf_link(
            "https://docs.example.com/doc.pdf", "PDF"
        )
    ...
    def test_links_by_domain(self) -> None:
        """Test grouping links by domain."""
        source_url = SourceUrl.from_string("https://example.com")
        link1 = ExtractedLink.create_pdf_link(
            "https://docs.example.com/doc1.pdf", "PDF1"
        )
        link2 = ExtractedLink.create_pdf_link(
            "https://docs.example.com/doc2.pdf", "PDF2"
        )
        link3 = ExtractedLink.create_other_link("https://other.com/page", "Page")

        result = ExtractionResult(
            source_url=source_url, pdf_links=[link1, link2], other_links=[link3]
        )
    ...
    def test_summary_properties(self) -> None:
        ...
        result = ExtractionResult(source_url=source_url, pdf_links=[pdf_link])
